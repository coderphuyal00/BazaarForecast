...
function ModalSearch({
  ...
  const { authTokens, user, userDetails } = useAuth();
  ...
  const [watchlist, setWatchlist] = useState(
    localStorage.getItem(`${userDetails?.first_name}_stockwatchlist`)
      ? localStorage.getItem(`${userDetails?.first_name}_stockwatchlist`)
      : null
  );
  ...
  const handleClick = async (e) => {
    ...
    if (clickedOn === "add") {
        ...
        if (suggestionId != "" && inputStockID != "") {
          try {
            let response = await fetch(
              "http://127.0.0.1:8000/api/user/watchlist/add/",
              {
                method: "POST",
                headers: {
      ...
      if (stockID != "") {
        try {
          let response = await fetch(
            `http://127.0.0.1:8000/api/user/watchlist/stock/${stockID}/delete/`,
            {
              method: "DELETE",
              headers: {
                Authorization: "Bearer " + authTokens?.access,
          ...
          if (response.status === 204) {
            console.log("Stock removed from watchlist list.");
            // console.log(data);
          } else {
            console.error("Error:", data);
          }
