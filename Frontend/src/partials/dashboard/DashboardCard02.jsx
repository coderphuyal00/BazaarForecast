...
const fetchDataOncePerDay = async () => {
  const cacheKey = "sensitiveChartData";
  const cacheTimestampKey = "sensitiveChartDataTimestamp";
  ...
  const response = await fetch("http://127.0.0.1:8000/api/index-price/sensitive/");
  if (!response.ok) {
